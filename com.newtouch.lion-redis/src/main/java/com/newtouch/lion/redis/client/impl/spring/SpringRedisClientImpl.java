...
import com.newtouch.lion.redis.util.CacheUtils;
 ...
 * 
 */
public class SpringRedisClientImpl extends DefaultSpringClientImpl implements RedisClient {
    ...
    protected <R> R performFunction(String key, CallBack<R> callBack) {
        return shardedJedis.getShard(key).execute(callBack);
    }
...
//    }
     ...
     * @see #sort(String, redis.clients.jedis.SortingParams)
     * @see#sort(String, redis.clients.jedis.SortingParams, String)
     * 
     * 
     * @param key specified key
     ...
     * Time complexity: O(1)
     * 
     * @seeredis.clients.jedis.Jedis#lpush(String, String)
     * 
     * @param key specified key
     * @param fields fields
     * @return Integer reply, specifically, the number of elements inside the list after the push operation.
     ...
     * {@link #(String, int, int) LRANGE} the result is sent to the client as a multi-bulk reply (see the protocol
     * specification for more information). If just a single key is specified, then this command produces the same
