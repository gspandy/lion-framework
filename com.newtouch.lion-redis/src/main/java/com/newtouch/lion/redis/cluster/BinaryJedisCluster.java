 ...
 * All rights reserved. 
 *
 * $id: BinaryJedisCluster.java 9552 2015年7月11日 下午10:20:49 WangLijun$
 */
...
/**
 * <p>
 * Title:Jedis Cluster 扩展
 * </p>
 * <p>
 * Description:Jedis Cluster 支持byte[]数组存储
 * </p>
 * <p>
...
public class BinaryJedisCluster extends JedisCluster {
	...
	public BinaryJedisCluster(JedisClusterConfig jedisClusterConfig) {
		super(jedisClusterConfig.getJedisClusterNode(), jedisClusterConfig
				.getTimeout(), jedisClusterConfig.getMaxRedirections(),
				jedisClusterConfig.getPoolConfig());
		this.connectionHandler = new JedisSlotBasedConnectionHandler(
				jedisClusterConfig.getJedisClusterNode(),
				jedisClusterConfig.getPoolConfig(),
				jedisClusterConfig.getTimeout());
		this.maxRedirections = jedisClusterConfig.getMaxRedirections();
	}
	...
	public Long del(final String key) {
		return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
			...
			public Long execute(Jedis connection) {
				return connection.del(SafeEncoder.encode(key));
			}
		}.run(key);
	}
	...
	public String setex(final String key, final int seconds, final byte[] value) {
		return new JedisClusterCommand<String>(connectionHandler,
			...
			public String execute(Jedis connection) {
				return connection
						.setex(SafeEncoder.encode(key), seconds, value);
			}
		}.run(key);
	}
	...
	public Long hset(final String key, final String field, final byte[] value) {
		return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
			...
			public Long execute(Jedis connection) {
				return connection.hset(SafeEncoder.encode(key),
						field.getBytes(Charset.defaultCharset()), value);
			}
		}.run(key);
	}
	...
	public byte[] hgetBytes(final String key, final String field) {
		return new JedisClusterCommand<byte[]>(connectionHandler,
			...
			public byte[] execute(Jedis connection) {
				return connection.hget(SafeEncoder.encode(key),
						field.getBytes(Charset.defaultCharset()));
			}
		}.run(key);
	}
	 ...
	 @Override
	  public Long hdel(final String key, final String... field) {
	    return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
	      @Override
	      public Long execute(Jedis connection) {
	        return connection.hdel(SafeEncoder.encode(key),SafeEncoder.encodeMany(field));
	      }
	    }.run(key);
	  }
	...
	public Long hsetnx(final String key, final String field, final byte[] value) {
		return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
			...
			public Long execute(Jedis connection) {
				return connection.hsetnx(SafeEncoder.encode(key),
						field.getBytes(Charset.defaultCharset()), value);
			}
		}.run(key);
	}
	...
	public Set<byte[]> zrangeBytes(final String key, final long start,
			final long end) {
		return new JedisClusterCommand<Set<byte[]>>(connectionHandler,
				maxRedirections) {
	...
	public Set<byte[]> zrangeByScoreBytes(final String key, final double min,
		...
		return new JedisClusterCommand<Set<byte[]>>(connectionHandler,
			...
			public Set<byte[]> execute(Jedis connection) {
				return connection.zrangeByScore(SafeEncoder.encode(key), min,
						max);
			}
		}.run(key);
	}
	...
	public Set<byte[]> zrevrangeByScoreBytes(final String key,
